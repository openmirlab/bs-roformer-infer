...
def test_device_contract(monkeypatch):
    ...
    with pytest.raises(RuntimeError): _select_device(Namespace(device="cuda:1"))
    with pytest.raises(ValueError): _select_device(Namespace(device="mps"))
...
def test_session_lifecycle_cache_resolver_and_cuda_forwarding(monkeypatch, tmp_path):
    ...
    checkpoint.write_bytes(b"x"); config.write_text("model: {}\n")
    ...
    class FakeModel:
        def load_state_dict(self, _state): pass
        def to(self, device): seen["to"] = device; return self
    ...
    monkeypatch.setattr("bs_roformer.utils.get_model_from_config", lambda *_a: constructed.append(1) or FakeModel())
    monkeypatch.setattr(torch, "load", lambda *_a, **_k: {})
    monkeypatch.setattr("bs_roformer.inference.torch.cuda.is_available", lambda: True)
    monkeypatch.setattr("bs_roformer.inference.torch.cuda.device_count", lambda: 2)
    monkeypatch.setattr("bs_roformer.inference.run_folder", lambda _m, _a, _c, device, **_k: seen.setdefault("run", device))
    session = BSRoformerSession(model_name="unknown", device="cuda:1")
    with pytest.raises(RuntimeError): session.infer(tmp_path)
    session.cache_info()
    ...
    session.release(); session.load()
    assert len(constructed) == 2
    session.close(); session.close()
    with pytest.raises(RuntimeError): session.load()
