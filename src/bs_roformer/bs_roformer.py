...
# NOTE: hyper_connections is NOT used because existing checkpoints were trained
# with older BS-Roformer without hyper-connections wrappers.
# Keeping the import for potential future use with newer checkpoints.
...
class Transformer(Module):
    def __init__(
        ...
        *,
        ...
        num_residual_streams = 1,  # kept for API compatibility, but not used
        num_residual_fracs = 1     # kept for API compatibility, but not used
    ):
        super().__init__()
        self.layers = ModuleList([])

        # Use simple residual connections (compatible with existing checkpoints)
        # Note: hyper_connections wrappers are NOT used because checkpoints were
        # trained with the older architecture without them.
    ...
    def forward(self, x, value_residual = None):
        ...
        for attn, ff in self.layers:
            # Simple residual connections (matching checkpoint architecture)
...
class BSRoformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.expand_stream and self.reduce_stream are identity functions.
